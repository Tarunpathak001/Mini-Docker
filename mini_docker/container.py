...
class Container:
    ...
    def create(
        ...
        **kwargs,
        ...
        # Extract seccomp setting
        seccomp_enabled = kwargs.get("seccomp_enabled", True)
    ...
    def _run_container(
        ...
        try:
            ...
            if namespaces_to_create:
                ...
                # If we created a new PID namespace, we must fork for it to take effect.
                # The first child (current process) becomes a "shim" that waits for
                # the grandchild (actual container PID 1).
                if "pid" in namespaces_to_create:
                    pid = os.fork()
                    if pid > 0:
                        # Shim process: forward signals and wait for child
