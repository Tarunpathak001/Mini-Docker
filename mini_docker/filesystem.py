...
def setup_chroot_filesystem(rootfs_path: str) -> None:
    ...
    try:
        ...
        print(f"Mount proc failed: {e}", file=sys.stderr)
...
def setup_pivot_root(merged_path: str) -> None:
    ...
    # Mount cgroup2
    cgroup_path = os.path.join(merged_path, "sys", "fs", "cgroup")
    os.makedirs(cgroup_path, exist_ok=True)
    try:
        mount("cgroup2", cgroup_path, "cgroup2", MS_NOSUID | MS_NOEXEC | MS_NODEV)
    except FilesystemError as e:
        print(f"Warning: Failed to mount cgroup2: {e}", file=sys.stderr)
    ...
    # Create /dev/pts
    pts_path = os.path.join(merged_path, "dev", "pts")
    os.makedirs(pts_path, exist_ok=True)
    try:
        mount("devpts", pts_path, "devpts", MS_NOSUID | MS_NOEXEC)
    except FilesystemError:
